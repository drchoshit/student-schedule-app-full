...
app.post("/api/orders/commit", async (req, res) => {
  try {
    ...
    console.error("POST /api/orders/commit error:", e);
...
app.get("/api/orders/by-student/:code", async (req, res) => {
  try {
    ...
    const s = await get("SELECT id, name, code FROM students WHERE code=?", [code]);
    if (!s)
      return res
        .status(404)
    ...
    const rows = await all(
      "SELECT id, date, slot, price, status FROM orders WHERE student_id=? ORDER BY date DESC, slot ASC",
      [s.id]
    );

    res.json({ ok: true, items: rows });
  } catch (e) {
    console.error("GET /api/orders/by-student error:", e);
    res
      .status(500)
      .json({ ok: false, error: String(e?.message || e) });
  }
...
app.use("/api", (req, res, next) => next()); // ✅ api 라우트를 먼저 통과시킴
